#! /bin/bash 
echo "######KernelSU的一键编译脚本，可重复执行，获得最新KernelSU版本######"
echo "           "
echo "           "
echo "脚本为国内加速构建脚本，没上网条件首选此脚本"
...
#添加KernelSU内核配置，当然已经在内核源内运行过一遍，可将此3项注释掉，多次执行也没问题指不过会在 dervierMarkfile,KD}_defcofig引入过多重复，无影响。。。。
