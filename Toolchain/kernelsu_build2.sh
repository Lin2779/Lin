...
echo "######KernelSU的一键编译脚本，可重复执行，需要魔法。获得最新KernelSU版本######"
...
echo "CONFIG_KPROBES=y" >> "$GKI_ROOT/arch/${ARCH}/configs/${KD}"
echo "CONFIG_HAVE_KPROBES=y" >> "$GKI_ROOT/arch/${ARCH}/configs/${KD}"

echo "CONFIG_KPROBE_EVENTS=y" >> "$GKI_ROOT/arch/${ARCH}/configs/${KD}"
...
if [ -f $GKI_ROOT/out/arch/${ARCH}/boot/${KERNEL} ]
	then
		...
		zip -r kernel-KernelSU_`date '+%Y%m%d%H%M'`.zip  . ;
		mv *.zip /mnt ;
		rm ${KERNEL} dtbo.img ;
		echo "           "
		echo "           "
