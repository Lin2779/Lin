...
export PATH="/workspaces/Lin/clang/bin:/workspaces/Lin/gcc/bin:$PATH"
...
make LLVM=1 CC="ccache clang" ${args} ${KD}
make LLVM=1 CC="ccache clang" ${args} 2>&1 | tee kernel.log



#预先清理一下/root/Toolchain/AnyKernel3/可能遗留的文件，防止打包混乱。
rm -rf /workspaces/Lin/Toolchain/AnyKernel3/*.zip /workspaces/Lin/Toolchain/AnyKernel3/Image /workspaces/Lin/Toolchain/AnyKernel3/Image.gz-dtb /workspaces/Lin/Toolchain/AnyKernel3/Image.gz /workspaces/Lin/Toolchain/AnyKernel3/dtbo.img 

# 打包内核，用AnyKernel3进行打包,最终内核成品在 /mnt目录下,名称为 kernel+日期的zip包，可通过twrp或ex内核管理器刷入，验证。
if [ -f ./out/arch/${ARCH}/boot/${KERNEL} ]
	then
		cp -rf ./out/arch/${ARCH}/boot/${KERNEL} /workspaces/Lin/Toolchain/AnyKernel3
		cd /workspaces/Lin/Toolchain/AnyKernel3 ;
		zip -r kernel-`date '+%Y%m%d%H%M'`.zip  . ;
		mv *.zip /workspaces/Lin/zip ;
		rm ${KERNEL} ;
		echo "           "
		echo "           "
